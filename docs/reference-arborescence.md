...
│   │                     `runAllocation(input)` — **chantier 91** : les tables se forment sur les **actifs** seulement, les passifs sont ensuite placés **en public** (`placeAudience` : uniformément sur les tables animées, limite `TABLE_TOTAL_MAX` = 30 personnes, tables enregistrables protégées des non-consentants ; `audience_member_ids`). 4 règles en **ordre lexicographique strict**, sur les actifs : ≥1 table enregistrable · hétérogénéité ≤70 %/2e camp ≥2 (maximin plafonné au seuil) · anciens ≥ ⌈2/5·actifs⌉ (plancher 3 sans modérateur) · nouveaux aux tables animées. Contraintes dures : actifs ≤10 → table unique ; sinon 5..14 actifs par table animée (répartis également), 5..7 sans modérateur.
...
│   │                     **Ne peut jamais échouer** : dégradation par l'ordre lexicographique (règle 4 sacrifiée d'abord, règle 1 en dernier). Recherche locale **déterministe** (graine fixe `DEFAULT_SEED`) et budget d'évaluations borné. Tests : `src/lib/allocation.test.ts` (`npm test`).
│   │                     **Chantier 25b (H17)** : un modérateur en surplus (plus de modérateurs que de tables animées) redevient un **participant ordinaire** et entre dans la population **avant** la recherche — il compte comme **actif** (chantier 91) et est donc optimisé par les règles 1 à 4 comme n'importe qui, jamais laissé sans affectation ni casé après coup. La circularité (asseoir quelqu'un change le nombre de tables, donc le surplus) est levée par **énumération** du nombre `k` de modérateurs qui animent : on part de `k = M` et on descend `k` **d'un cran à la fois** (chantier 91 : sauter directement au nombre de tables produit manquait le bon `k`) jusqu'au premier `k` cohérent (`k ≤ T`). `k = 0` l'est toujours. Entrée `moderatorProfiles` = attributs réels de ces modérateurs (toujours fournie par `loadAllocationInputs`). Sorties : `seatedModeratorIds`, `animatingModerators`, `recorderTarget`.
...
│   ├── AllocatingScreen.tsx    ⚠️ **Nom trompeur : monté seulement à la phase `debating`, jamais pendant que `sessions.phase === 'allocating'`.** Pendant la phase `allocating` elle-même, le participant reste sur `VoteScreen` (step `'vote'`, bannière ambre "L'organisateur forme les groupes") — `VoteScreen.tsx` ne route vers ce composant (`setStep('allocating')`) que sur `phase === 'debating'`, voir son commentaire "on reste sur le vote avec bannière" (~ligne 225). Découvert au chantier 122 en testant au navigateur ; à garder en tête pour tout futur chantier qui voudrait afficher quelque chose dès la vraie phase `allocating` — il faudra toucher `VoteScreen.tsx`, pas ce fichier. // Affectation groupe, code table, nom du camp (DB via session.group_names en priorité, localStorage fallback), bouton rejoindre. Carte "Ton binôme" (lecture seule, `getMyPairings` + `PairingResultsList`) et nudge documentaire (`DocNudge`, liens fiche info/résumé/collab/biais cognitifs/arguments fallacieux) — chantier 122. Affiche VoteResultsSummary + accordéon "Voir toutes les assertions"
