...
### 120 — `App.tsx` : le renouvellement du jeton anonyme peut désynchroniser `session_members.user_id`

> 🔍 Découvert en aparté au chantier 119 (pas un correctif — un signal détecté en relisant `sync_table_assignment` pendant l'audit du code de rappel), non confirmé par un repro réel. À vérifier avant de coder quoi que ce soit.
