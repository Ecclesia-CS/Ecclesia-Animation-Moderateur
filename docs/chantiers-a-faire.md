...
> **Retour de Jules (2026-09-21)**, après avoir testé le chantier 117 : « quand je tente de cliquer sur "enlever" ou "libérer la modération de cette table" ce n'est pas possible, il y a marqué "membre introuvable pour cette séance". Je pense que les modérateurs ne sont pas encore traités comme des participants, alors que le but, c'est que quand ils ne tiennent plus la modération d'une table (l'écran modérateur), ils soient comme des participants normaux, avec des codes, des votes, etc, mais juste flagué modérateur pour que le superadmin sache qu'ils sont là s'il a besoin d'eux. C'est dans la continuité de différencier modérateur d'une table, et modérateur flagué qu'on avait déjà fait dans un autre chantier. »

**Sur l'erreur observée** : à vérifier en premier, avant de coder quoi que ce soit — le bundle déployé au moment du test contenait bien le correctif attendu (vérifié en relisant le JS minifié servi par GitHub Pages, `onClick:()=>Xe.member_id?$o(Xe.member_id):G.table_id&&ri(G.table_id)`, qui route bien vers `handleReleaseTableModeration` — pas `handleRemoveTableModerator` — quand `member_id` est `null`), et `release_table_moderation` ne contient, en base, aucun `RAISE` portant ce message. L'hypothèse la plus probable est un onglet superadmin resté ouvert depuis avant le déploiement (SPA statique, le nouveau bundle ne remplace pas un JS déjà chargé en mémoire sans rechargement complet) : l'ANCIEN bundle appelait `handleRemoveTableModerator(mod.member_id)` sans condition, y compris pour ce modérateur physique — `member_id = null` contre `session_members` ne matchait donc jamais (`set_member_moderator` lève exactement ce message quand `NOT FOUND`). Un `assignError` qui ne se réinitialisait pas après un succès (corrigé dans ce même commit, cf. `docs/chantiers.md` chantier 117) explique aussi qu'un second clic ait pu sembler échouer avec le même message alors qu'il avait réussi. **Première étape de ce chantier : demander à Jules de rejouer le test après un rechargement complet (Ctrl+Maj+R) de l'onglet superadmin**, pour confirmer que le problème persiste réellement une fois ce doute levé — sinon ce chantier peut se limiter à la partie ci-dessous, qui reste pertinente indépendamment du bug d'affichage.

**Le vrai problème, indépendant du bug ci-dessus** : `claim_table_as_moderator` (chantier 68/72) fait entrer quelqu'un dans le débat **entièrement hors du modèle Bloc C** — aucune ligne `session_members` n'est jamais créée. Tant qu'il anime sa table, chantier 117 le rend visible par une ligne synthétique (`member_id = NULL`) dans `list_table_assignments_admin`, mais cette ligne n'est qu'un affichage : aucune action qui prend un `member_id` en paramètre (`set_member_moderator`, la bascule modérateur de l'onglet Membres) ne peut le cibler. Et `release_table_moderation`, seul levier qui s'applique réellement à lui (il agit au niveau TABLE), ne fait que couper son autorité (`tables.created_by`) — il redevient un `participants` totalement invisible du superadmin, sans code de rappel, sans pouvoir voter, sans plus aucune trace dans `session_members`. C'est l'inverse de ce que Jules demande : il doit **rester visible et geré comme un participant normal, flagué modérateur**, pas disparaître.

**Rapprochement avec l'existant, à ne pas manquer** : le chantier 106 a déjà posé exactement ce concept pour les modérateurs Bloc C — un modérateur qui n'anime aucune table garde son drapeau `is_moderator = true` mais devient un participant ordinaire de sa table (« modérateur en surplus », distingué de « modérateur en exercice » via `tables.active_moderator_member_id`). La demande de Jules ici, c'est très exactement de faire rentrer le modérateur physique dans **ce même schéma** plutôt que d'en inventer un second.

**Deux options d'implémentation, à trancher avant de coder** (ou à soumettre à Jules) :
- **Option A (paresseuse)** — ne rien faire tant que le modérateur anime sa table (le hack d'affichage du chantier 117 reste nécessaire pour cette fenêtre-là) ; au moment où `release_table_moderation` lui retire l'autorité, **créer à la volée** une ligne `session_members` (`user_id`, `pseudo` repris de sa ligne `participants`, `is_moderator = true`), un code de rappel neuf (haché, cf. chantier 93 — `regenerate_reclaim_code_admin` comme modèle), et une ligne `table_assignments` le rattachant à sa table actuelle (`table_number` déjà connu sur `tables.table_number`) — il devient ainsi immédiatement un « modérateur en surplus » assis là, exactement le schéma du chantier 106.
- **Option B (à la source)** — créer cette même ligne `session_members` (+ `table_assignments`, + code de rappel) **dès `claim_table_as_moderator`**, au moment où la personne prend la table, plutôt que d'attendre son retrait. Il devient un membre Bloc C dès son arrivée, avec `active_moderator_member_id` posé sur lui (déjà le cas, chantier 106 §1 de ses cinq chemins listés) — il est donc "en exercice" tout de suite. Ceci **supprime le besoin du hack du chantier 117** (`list_table_assignments_admin` peut revenir à sa version chantier 106, sans la branche `UNION ALL`) puisqu'il a une ligne `session_members`/`table_assignments` dès le départ. Plus simple et plus cohérent avec le reste du modèle, mais change le comportement de `claim_table_as_moderator` : quelqu'un qui prend une table par Code Ecclesia devient de fait « inscrit à la séance », même s'il n'a jamais voté — à confirmer avec Jules que c'est bien voulu (il ne semble pas y avoir de raison de refuser : c'est un participant réel du débat).

**Points d'attention pour l'implémentation, quelle que soit l'option retenue** :
- Le code de rappel est **haché** depuis le chantier 93 (`reclaim_code_hash`) — impossible de le générer sans le montrer explicitement à cet instant à l'écran (comme `regenerate_reclaim_code_admin`/`_self`, chantier 116) ou de le confier au modérateur d'une autre façon (il n'a pas d'écran d'inscription classique à ce moment).
- Collision de pseudo : `UNIQUE(session_id, pseudo)` sur `session_members` (à vérifier dans `docs/reference-modele-donnees.md`) — le pseudo du `participants` existant peut déjà être pris par un autre membre de la séance (peu probable mais possible, notamment si plusieurs personnes portent le même prénom).
- `attending_in_person` : poser `true` (il est physiquement là).
- Ne pas casser `is_table_moderator` (chantier 60) : les deux branches cumulatives (créateur physique OU `is_moderator` + `active_moderator_member_id`) doivent continuer de s'appliquer sans double-comptage une fois que ce modérateur a les deux représentations en même temps (transitoirement, le temps qu'`Option A/B` s'applique).
- **Règle SQL** : `claim_table_as_moderator` et/ou `release_table_moderation` sont des fonctions existantes → comparer à `pg_get_functiondef` en base avant de réécrire (dernière définition connue : `20260906_chantier72_1_reprise_moderation.sql` pour la première, ce chantier-ci pour la seconde).
- **Fichiers** : `supabase/migrations/…`, `src/screens/SuperadminScreen.tsx` (si l'option A/B change ce qui doit encore être filtré des diagnostics d'allocation), potentiellement retrait du correctif du chantier 117 si l'option B est retenue.

### 120 — `App.tsx` : le renouvellement du jeton anonyme peut désynchroniser `session_members.user_id` — ⚠️ BUG CONFIRMÉ, bloqué sur un arbitrage de Jules
...
>
> **Pas de correctif codé ici** : les deux pistes de la spec ci-dessous ont un vrai coût opposé — (a) réassigner `user_id` par pseudo (comme `join_table` le fait déjà sur `participants`) rouvrirait sur `session_members` le vecteur de prise d'identité par simple pseudo que le chantier 93 avait justement fermé (protection par `reclaim_code_hash`) ; (b) exiger une reconnexion explicite (pseudo + code, via `confirm_attendance`) préserve cette protection mais casse la continuité silencieuse actuelle, sans écran disponible à ce stade de `App.tsx` (avant tout rendu), et suppose que l'utilisateur ait encore son code. **Décision de sécurité/produit à trancher avec Jules avant de coder quoi que ce soit** — ce n'est pas un arbitrage technique.
