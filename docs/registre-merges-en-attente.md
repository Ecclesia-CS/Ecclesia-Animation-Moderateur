...
|---|---|---|---|
| `chantier-59-realtime-prive` | **Mergé sur `main` le 2026-09-07** (commit `c2d06c7`, tag `pre-merge-chantier59-20260907`), sur demande explicite de Jules d'avancer avant la séance du 10/09 (dérogation à l'avertissement d'origine ci-dessous, resté volontairement affiché). Migration appliquée en base le 2026-09-07 (corps de `is_table_participant` vérifié contre `pg_get_functiondef` avant application). Recette navigateur **partielle seulement** — voir `A_VERIFIER.md`, chantier 59 : jointure et absence de `CHANNEL_ERROR` confirmées avec un seul `auth.uid()` sur deux onglets, mais aucun vrai test à deux identités distinctes, et les scénarios C à I pas déroulés. **« Allow public access » n'a PAS été désactivé** (réglage dashboard, hors SQL/MCP, pas de session authentifiée disponible pour cette session) — ne pas le faire avant d'avoir rejoué la recette complète avec deux identités réelles. | **Appliquée** (2026-09-07). | ~~Trop risqué avant la séance du 10/09. À merger après.~~ Fait le 07/09 sur instruction de Jules. **Ordre d'application non négociable, partiellement suivi** : 1. migration appliquée ✅ 2. code déployé ✅ 3. recette — **partielle** ⚠️ 4. désactivation « Allow public access » — **pas fait, bloquant, à faire seulement après une recette complète**. Rollback d'urgence si besoin : réactiver le réglage (déjà actif aujourd'hui), effet immédiat sans redéploiement. |
...
| `claude/chantier-91-allocation-passifs` | Chantier 91 — allocation : tables formées sur les actifs, passifs placés en public (uniformément sur les tables animées, 30 personnes max par table), règle « assez d'actifs » supprimée. Touche `src/lib/allocation.ts` (+ tests, banc d'essai), `TableDiagnosticsList.tsx`, `AllocationPanel.tsx`, `SuperadminScreen.tsx` (onglet Groupes). | Aucune. | **En attente de validation de Jules** (changement algorithmique : pas de merge sans son accord explicite sur `docs/chantier-91-comparatif-allocation.md`). Recette navigateur non jouée. **Le chantier 92 doit attendre ce merge** (même fichier). |
