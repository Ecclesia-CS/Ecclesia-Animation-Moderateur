...
|---|---|---|---|
| ~~`chantier-59-realtime-prive`~~ | **Chantier clos le 2026-09-16.** Mergé sur `main` le 2026-09-07 (commit `c2d06c7`, tag `pre-merge-chantier59-20260907`), migration appliquée le 2026-09-07, recette rejouée en profondeur le 2026-09-16 avec deux vraies identités distinctes — voir `A_VERIFIER.md`, chantier 59. **Jules a désactivé « Allow public access » le 2026-09-16** (Realtime → Settings, pas Project Settings). Scénario B rejoué immédiatement après : canal public → `CHANNEL_ERROR`/"PrivateOnly: This project only allows private channels" ; canal privé légitime → toujours `SUBSCRIBED`, aucune régression. **F6 est fermé.** | **Appliquée** (2026-09-07). | ~~Trop risqué avant la séance du 10/09.~~ Fait le 07/09, recette complétée et « Allow public access » désactivé le 16/09. **Les 4 étapes de l'ordre d'application sont faites.** Rollback d'urgence si besoin en séance réelle : réactiver le réglage dans le dashboard, effet immédiat sans redéploiement. |
...
`chantier-58-colonnes-sessions` est protégée en dur par `scripts/cleanup-worktrees.sh` — ne pas la retirer de sa liste d'exclusion. `chantier-secu-sauvegardes` y figurait aussi mais a été retirée de la liste le 2026-09-20, la branche étant mergée sur `main` (chantier 85).
