...
| `claude/chantier-91-allocation-passifs` | Chantier 91 — allocation : seuil 3/5, 14 actifs par table, passifs aux tables animées, nouvelle règle 2 « passifs encadrés ». Touche `src/lib/allocation.ts` (+ tests), `TableDiagnosticsList.tsx`, `AllocationPanel.tsx`, `SuperadminScreen.tsx` (libellés seulement). | Aucune. | **En attente de validation de Jules** (changement algorithmique : pas de merge sans son accord explicite sur le tableau avant/après, `docs/chantier-91-comparatif-allocation.md`). Recette navigateur non jouée. **Le chantier 92 doit attendre ce merge** (même fichier). |
