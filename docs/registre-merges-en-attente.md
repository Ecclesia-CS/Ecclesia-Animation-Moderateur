...
> **2026-09-15 — le 10/09 est passé, cette section est périmée.** Personne ne l'a supprimée entre-temps ; laissée telle quelle par prudence (append-only) plutôt que retirée sans confirmation. Le gel ne s'applique plus à aucune branche à partir de maintenant.
...
|---|---|---|---|
| `chantier-59-realtime-prive` | **Mergé sur `main` le 2026-09-07** (commit `c2d06c7`, tag `pre-merge-chantier59-20260907`). Migration appliquée en base le 2026-09-07. **Recette rejouée en profondeur le 2026-09-16** avec deux vraies identités distinctes (Jules + session Claude Code) — voir `A_VERIFIER.md`, chantier 59 : les 8 topics de la carte d'autorisation testés individuellement (`table`, `allocating`, `vote`, `collab`, `session-member`, `postvote`), propagation temps réel confirmée (bascule de parole sur table leaderless), frontière d'autorisation vérifiée dans les deux sens (légitime accepté, illégitime refusé, anonyme refusé). Scénario H (non-régression superadmin) partiellement concluant — RPC de lecture toutes vérifiées à 200, mais le levier précis du polling de recalcul de groupe pas isolé à temps. **« Allow public access » toujours PAS désactivé** — en attente du feu vert explicite de Jules (lui seul a accès au dashboard ; la session Claude Code refuse par principe de saisir un mot de passe dans un formulaire, même celui de sa propre app). | **Appliquée** (2026-09-07). | ~~Trop risqué avant la séance du 10/09. À merger après.~~ Fait le 07/09 sur instruction de Jules, recette complétée le 16/09. **Ordre d'application non négociable** : 1. migration appliquée ✅ 2. code déployé ✅ 3. recette — **quasi complète** ✅ (H partiel) 4. désactivation « Allow public access » — **pas fait, en attente de Jules**. Rollback d'urgence si besoin : réactiver le réglage (déjà actif aujourd'hui), effet immédiat sans redéploiement. Bon chemin dashboard (trouvé le 16/09, corrige une doc erronée) : **Realtime → Settings**, pas Project Settings. |
| `chantier-58-colonnes-sessions` | **Mergé sur `main` le 2026-09-07** (branche `claude/restrict-session-public-columns-48a6fb`) et **`REVOKE`/`GRANT` appliqué le 2026-09-07 via MCP** (`onboarding_enabled` inclus dans la liste accordée), sur demande explicite de Jules d'avancer avant la séance du 10/09 (même dérogation que le chantier 59 ci-dessus). Les 4 RPC de lecture étaient déjà en base ; le `GRANT SELECT` restreint (`id, title, phase, join_code, scheduled_at, created_at, onboarding_enabled`) est désormais actif. | **Appliquée** (2026-09-07). | ~~Le `REVOKE` casserait huit écrans tant que le code correspondant n'est pas déployé.~~ Code mergé/déployé le 07/09, migration appliquée le 07/09, recette de vérification en pied de fichier déroulée intégralement via `curl`/MCP (détail dans `A_VERIFIER.md`, chantier 58) — colonnes annexes fermées (`42501`), colonnes publiques et 4 RPC fonctionnelles, `list_sessions_admin` refuse un mauvais mot de passe. **Reste à faire, à part** : un test navigateur réel du parcours participant/superadmin (non fait — `.claude/launch.json` absent du dépôt malgré la mention dans `CLAUDE.md`). |
| ~~`claude/chantier-91-allocation-passifs`~~ | Chantier 91 — allocation : tables formées sur les actifs, passifs placés en public (uniformément sur les tables animées, 30 personnes max par table), règle « assez d'actifs » supprimée. Touche `src/lib/allocation.ts` (+ tests, banc d'essai), `TableDiagnosticsList.tsx`, `AllocationPanel.tsx`, `SuperadminScreen.tsx` (onglet Groupes). | Aucune. | **Mergé sur `main` le 2026-09-16**, après validation explicite de Jules du tableau avant/après. Recette navigateur non jouée. Le chantier 92 peut partir de `main`. |
...
| `chantier-89-phase-post-vote` | Nouvelle phase `post_voting` entre `debating` et `closed` — le revote post-débat (chantier 69) devient une fenêtre fermée par le passage en `closed`, au lieu d'un accès permanent. Touche `src/lib/types.ts`, `phaseLabels.ts`, `SuperadminScreen.tsx`, `ResultsMapScreen.tsx`, `VoteScreen.tsx`, `AllocatingScreen.tsx`, `ParticipantView.tsx`, `ModeratorView.tsx`, `SessionRouterScreen.tsx`, `TableAssignmentCard.tsx`, `SessionQuestionnaireForm.tsx`. | **Appliquée** (`20260907_chantier89_post_voting_phase.sql`, 2026-09-07, via MCP). | **⚠️ Touche `VoteScreen.tsx`, en plein dans le gel §1 ci-dessus** — ne pas merger avant la fin de la séance du 10/09, même si le reste du chantier est prêt. **Vérifié au navigateur le 07/09** (séance QA jetable) — `debating → post_voting → closed`, questionnaire forcé, revote, disparition du bouton en `closed` : tout confirmé. Reste non couvert : le bouton `PhaseBar` superadmin lui-même (mot de passe non transmis à la session) et le visiteur non inscrit en `post_voting`. Détail dans `A_VERIFIER.md` § Chantier 89. |
| ~~`claude/dette-heritee-annexe-b-aa861e` (chantier 88a/88b/88c)~~ | **Mergé sur `main` le 2026-09-15** (commit `4e0394f`, merge `--no-ff`). Dette héritée (annexe B) : toasts (`ToastContext`), page 404 (`NotFoundScreen`), QR code superadmin dans `ShareLinkBanner`. Ne touche pas `VoteScreen.tsx`. | Aucune. | ~~Pas encore mergée.~~ Fait. **Vérifié au navigateur en intégralité** : D1, A2 (DnD), A3 (notes), page 404, les 3 scénarios de toast (exclusion, suppression de table en Realtime, restauration `localStorage` introuvable), et **le bouton QR code superadmin confirmé par Jules lui-même** le 2026-09-15 (`http://localhost:5173` local, séance réelle « Devons nous, et comment, réguler les réseaux sociaux ? »). Détail dans `A_VERIFIER.md` § Chantier 88a/88b/88c. C7 (bug affichage prévote, annexe B) et C2 (branding) volontairement **non traités** — C7 faute de repro trouvable nulle part dans le dépôt, C2 bloqué sur une charte graphique de Jules. Les deux restent ouverts en annexe B de `docs/chantiers.md`. |
...
## 3 bis. Ce qui a été mergé le 2026-09-07

`main` est passé de `1c62418` à `2174f08` (chantiers **74**, **76** et **77**), puis a reçu le chantier **78**. Tag de rollback posé avant ce dernier merge : `pre-merge-chantier-78-20260907`.

- **Chantier 74** — modale des phases au clic sur `PhaseIndicator` ; correctif du champ de saisie invisible à l'ajout d'une personne sans téléphone. Migration `add_offline_participant` appliquée le 06/09.
- **Chantier 76** — `docs/2026-09-06-plan-securite-consolide.md`, les quatre documents d'audit relus contre le code et la base. Documentaire.
- **Chantier 77** — `docs/chantiers.md`, index chronologique des chantiers 34 à 77. Documentaire.
- **Chantier 78** — ménage documentaire. `CLAUDE.md` **615 → 298 lignes**, découpé en sept annexes `docs/reference-*.md` (contenu déplacé, rien supprimé — vérifié ligne à ligne). `PROJECT_STATUS.md` **supprimé** : son contenu unique (les tâches lettrées des chantiers 1 à 25, soit 83 identifiants type `B3`/`D7`/`F24`/`H26` encore cités partout, et les 6 items encore ouverts) est passé en **annexes A et B de `docs/chantiers.md`**, qui devient le **seul** fichier de suivi de l'avancement. La passe de validation de Jules du 06/09 (**37** entrées, pas 33) est **réintégrée dans `A_VERIFIER.md`**, recettes complètes conservées, dont **12 portant la réserve « validé avant la refonte des chantiers 73/74, à revalider »**. Suppression de `docs/ecclesia_handoff_20260803.md` et des quatre plans/specs de transcription *live* de `docs/superpowers/`.

> Les **quatre documents d'audit de sécurité** (`audit-securite-2026-08-03`, `audit-securite-passation`, `2026-09-02-audit-securite-revue`, `2026-09-02-plan-action-securite`) ont été **délibérément conservés**, contre la consigne initiale du chantier 78. Vérification faite constat par constat : le plan consolidé du 06/09 couvre bien tous leurs constats, mais il ne les reproduit pas intégralement (il perd la méthodologie des tests actifs du 03/08 contre la production et les preuves d'exploitation), et il écrit lui-même en tête « ils ne sont pas supprimés, ils restent l'historique de comment on en est arrivé là » en pointant vers les quatre.
...
- `CLAUDE.md` est réinjecté au démarrage de **chaque** session : coût fixe payé tous les jours. **Fait (chantier 78, 2026-09-07)** : découpé en un cœur court plus sept annexes `docs/reference-*.md` liées, **615 → 298 lignes**. (Ce paragraphe annonçait « environ 1400 lignes » : c'était faux, le fichier n'en a jamais fait autant — 615 avant le découpage, 92 Ko. Le coût était réel, son ordre de grandeur surestimé du double.)
