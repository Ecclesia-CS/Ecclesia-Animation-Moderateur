...
| `claude/dette-heritee-annexe-b-aa861e` (chantier 88a/88b/88c) | Dette héritée (annexe B) : toasts (`ToastContext`), page 404 (`NotFoundScreen`), QR code superadmin dans `ShareLinkBanner`. Rebasée sur `origin/main` le 2026-09-15 (chantiers 58/59 inclus, un conflit d'imports résolu dans `TableContext.tsx`). Ne touche **pas** `VoteScreen.tsx`. | Aucune. | Pas encore mergée, juste poussée — aucun blocage connu. **Vérifié au navigateur** (2026-09-08 puis 15/09 après rebase) : D1, A2 (DnD), A3 (notes), page 404, et les 3 scénarios de toast (exclusion, suppression de table en Realtime, restauration `localStorage` introuvable). **Non vérifié** : le bouton QR code superadmin lui-même (mot de passe non transmis à la session). Détail dans `A_VERIFIER.md` § Chantier 88a/88b/88c. C7 (bug affichage prévote, annexe B) et C2 (branding) volontairement **non traités** dans cette branche — C7 faute de repro trouvable, C2 bloqué sur une charte graphique de Jules. |
