...
Ci-dessous, les items D1/A2/A3/A4+D17/C7/C2 du tableau plus haut, mis à jour au chantier 88 : **D1** (règles à l'entrée de table, `DebateRulesModal.tsx`), **A2** (bug DnD, corrigé au chantier 8) et **A3** (sauvegarde des notes, `NotesModal.tsx`) sont déjà en place — seule leur vérification au navigateur reste ouverte (`A_VERIFIER.md`). **A4+D17** (fin de séance / forçage questionnaire) a été traité autrement par les chantiers 39 et 89 (phase `post_voting`). **C7** (bug affichage prévote) touche `VoteScreen.tsx`, gelé jusqu'au 10/09 inclus (`docs/registre-merges-en-attente.md` §1) — non repris ici. **C2** (identité visuelle) reste bloqué sur une charte graphique à fournir par Jules.
