...
| 79 | Écran de comparaison avant/après débat | Nouvel accordéon « Comparaison avant / après débat » dans l'onglet 📊 Analyse : apparie les camps de deux analyses (chantier 70 : `vote_scope` `pre_closure`/`current`) par **composition réelle de membres** (`pairGroups`, glouton par chevauchement), jamais par `group_id` — deux calculs k-means successifs ne numérotent pas les camps pareil. Affiche mouvement des membres entre camps, appariement avec effectifs, assertions dont le consensus a le plus bougé (comparaison directe, indépendante du numéro de groupe). **Point laissé ouvert** : aucun bouton superadmin ne déclenche encore le calcul d'une analyse `pre_closure` — sans ça l'écran n'a jamais de second terme à comparer en pratique, voir `A_VERIFIER.md`. | 🟡 Livré, pas mergé — aucune vérification navigateur (consigne : pas de serveur de dev sans autorisation). Branche `chantier-79-comparaison-avant-apres`. | `src/components/AnalysisPanel.tsx` (`AnalysisComparisonPanel`), `src/lib/analysis.ts` (`pairGroups`) |
