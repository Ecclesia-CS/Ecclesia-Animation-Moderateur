...
| 91 | Allocation : seuil 3/5, 14 actifs par table, passifs encadrés — **livré sur branche, en attente de validation de Jules** | Consigne de Jules du 16/09. Seuil d'actifs `⌊3/5·taille⌋` (le `min(…, 4)` qui le neutralisait au-delà de 10 personnes est retiré) ; 14 actifs au plus par table animée, passifs en sus (plafond dur de 24) et d'abord sur les tables animées les plus fournies en actifs ; nouvelle règle 2 « passifs encadrés » juste après la règle 1 (les règles 2 à 5 deviennent 3 à 6). Trois ajustements décidés pendant l'implémentation, chacun pour corriger un défaut mesuré : manque d'actifs sans modérateur compté d'abord, le plus de tables animées possible, plafond de 24 personnes. Grandes salles nettement mieux réparties (200 part. : 10 tables au lieu de 40, 56 ms au lieu de 1,5 s). Aucune migration. | ⏳ Branche `claude/chantier-91-allocation-passifs`, **non mergée** — validation de Jules sur `docs/chantier-91-comparatif-allocation.md` requise, recette navigateur non jouée | `src/lib/allocation.ts`, `src/components/voting/TableDiagnosticsList.tsx` |
