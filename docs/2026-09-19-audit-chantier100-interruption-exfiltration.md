...
1. **`REVOKE EXECUTE ON FUNCTION ... FROM anon, authenticated`** sur les cinq fonctions du §2, plus les trois helpers `is_table_participant` / `is_table_moderator` / `is_own_session_member` et `generate_session_join_code`, tous inutilisés par `src/` (grep). Coût : une migration d'une dizaine de lignes, aucun changement de frontend, aucun risque de casse silencieuse — les fonctions restent appelables depuis les RPC `SECURITY DEFINER` qui les utilisent en interne. **C'est le meilleur rapport effet/risque du dossier après les sauvegardes.**
