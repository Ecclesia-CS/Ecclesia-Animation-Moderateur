...
- **Pas de bascule arrière automatique** : retirer un modérateur (`set_member_moderator(..., false)`) laisse la table `leaderless = false`, sans modérateur assis — état déjà supporté par l'app (table fraîchement créée par `create_tables_batch`, ou en cours de configuration en `allocating`). Un retour à `leaderless = true` n'est déclenché que par un recalcul complet de l'allocation (`apply_allocation`, `leaderless = NOT v_moderated`) — jamais par ce chemin de retrait. Voir `A_VERIFIER.md` (chantier 64) pour le risque documenté si le retrait a lieu en pleine phase `debating`.
