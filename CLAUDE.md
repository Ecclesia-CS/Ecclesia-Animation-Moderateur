...
└── components/
    ...
    │   │                             **Chantier 25/25b** : la proposition et les saisies sont persistées en `sessionStorage` (clé `ecclesia_alloc_preview_<sessionId>`, jamais en base) — elles survivent au changement d'onglet et au rechargement (H14). Liste à cocher des modérateurs présents : **décocher appelle `set_member_moderator(..., false)` et retire réellement le statut en base** ; la personne redevient un membre ordinaire et reste affichée dans la liste pour pouvoir être recochée (H16). Une **signature des entrées** est enregistrée au calcul : si les réglages changent ensuite, un bandeau s'affiche et « Appliquer » est désactivé jusqu'au recalcul. Horodatage « calculé à HH:MM:SS » (H13) et objectif d'enregistrement effectif affiché (H15).
