...
│   │                     **Chantier 25 (H17)** : un modérateur en surplus (plus de modérateurs que de tables animées) est placé comme **participant ordinaire** dans la table la moins remplie, jamais laissé sans affectation. Le placement se fait **après** la recherche de forme (`solveFor`) — réinjecter ces personnes dans la population en amont est circulaire (asseoir quelqu'un change le nombre de tables, donc le surplus) et dégrade fortement le résultat. Entrée facultative `moderatorProfiles` = attributs réels de ces modérateurs, pour qu'ils comptent dans les seuils de leur table. Sorties associées : `seatedModeratorIds`, `animatingModerators`, `recorderTarget`.
...
└── components/
    ...
    │   │                             **Chantier 25** : la proposition et les saisies sont persistées en `sessionStorage` (clé `ecclesia_alloc_preview_<sessionId>`, jamais en base) — elles survivent au changement d'onglet et au rechargement (H14). Liste à cocher des modérateurs présents : un décoché est réinjecté dans `members` comme participant ordinaire (H16). Horodatage « calculé à HH:MM:SS » pour que relancer soit visible même à résultat identique (H13), et objectif d'enregistrement effectif affiché sous le champ (H15).
...
- **Réinjecter les modérateurs en surplus dans la population *avant* la recherche de forme** — le problème devient circulaire (asseoir quelqu'un augmente la population → change le nombre de tables → change le surplus → …) et le résultat se dégrade fortement : sur 30 participants / 4 modérateurs, cette approche transforme `[10, 10, 10]` en 6 tables. Le placement du surplus se fait **après** `solveFor`, sur la forme retenue (chantier 25 / H17).
