...
**Migration non appliquée** (MCP Supabase indisponible cette session — confirmé via `ToolSearch`, aucun outil `mcp__supabase__*`). SQL complet dans le fichier de migration ci-dessus, à appliquer manuellement par Jules. Aucun changement frontend nécessaire (le chemin UI testé était déjà correct) — `npx tsc -b` et `npm run build` passent sans erreur (aucune modification TypeScript dans ce chantier).
