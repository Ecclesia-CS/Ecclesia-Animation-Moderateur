...
## Chantier 107 (2026-09-21) — `claim_moderator_status` ne doit jamais déplacer quelqu'un déjà assis — ✅ vérifié en base (SQL direct, transaction annulée), ⚠️ non rejoué au navigateur
...
**Non testé par cette vérification SQL, à rejouer au navigateur avant de considérer le chantier définitivement clos** :
- Le cas central de l'audit (celui qui a motivé le chantier) : un membre déjà assis sur une table animée **différente** de la première table libre sans modérateur — vérifier qu'il ne bascule pas sur cette dernière et garde ses actions d'animation fonctionnelles sur sa propre table. Le scénario SQL ci-dessus ne le distingue pas de l'ancien comportement car il n'y avait qu'une seule table libre candidate ; à rejouer avec au moins deux tables candidates libres pour être sûr que le nouveau code ne réaffecte vraiment rien.
- Recalcul d'allocation après une déclaration en `allocating` : vérifier que le modérateur fraîchement déclaré est bien pris en compte par `get_allocation_inputs`/`runAllocation` au clic sur "Recalculer" (neutralité tranchée par Jules — pas de filtrage ajouté côté algorithme, mais jamais rejoué de bout en bout).
- Parcours front réel (bouton "Je suis modérateur" côté participant) — cette session n'a touché que la migration SQL, aucun fichier de `src/`.
