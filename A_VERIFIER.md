...
- **✅ Vérifié au navigateur le 2026-09-07** (worktree principal, jeton de serveur de dev tenu le temps du test) — recette : séance QA jetable créée par SQL direct (`🧪 Chantier 89 — test phase post_voting`, `join_code=QA89A1`, phase `voting`, `onboarding_enabled=false`, purgée en fin de test), inscription d'un vrai participant via `#vote/QA89A1` (formulaire réel, pas de bypass), assertion de test votée `agree` via l'UI, puis phase avancée à la main par `UPDATE sessions SET phase=...` (je n'ai pas le mot de passe superadmin — Jules a préféré le garder — donc les transitions elles-mêmes n'ont **pas** été déclenchées par le bouton `PhaseBar`/`handlePhaseChange`, seul le comportement **frontend** en aval de `sessions.phase` a été exercé) :
  - [x] `debating` (Realtime, sans reload) : `PhaseIndicator` passe à "Étape 4 · Débat", `TableAssignmentCard` affiche bien le message de sortie de secours "tu n'as pas encore de table" (comportement inchangé, attendu vu qu'aucune allocation n'a eu lieu).
  - [x] `post_voting` (Realtime, sans reload) : `PhaseIndicator` passe à "Étape 5 · Post-débat" et **le questionnaire post-débat se force automatiquement**, sans action superadmin explicite côté RPC — confirme que le frontend déclenche bien ce chemin dès qu'il observe `phase === 'post_voting'` en base (`VoteScreen`, `hasQuestionnaireResponse`). Après soumission → `ResultsMapScreen` avec le bandeau "Le débat a peut-être changé ton avis." et le bouton **"↻ Revoter" visible**.
  - [x] `PostVoteScreen` : ouverture depuis "↻ Revoter", section 1 affiche l'assertion propre avec son vote existant, changement de vote via la modale "Changer mon vote" — vérifié en base (`assertion_votes.vote` passé de `agree` à `disagree`) et reflété dans l'UI (icône ❌).
  - [x] `closed` (après reload — `ResultsMapScreen` ne s'abonne pas au Realtime sur `sessions`, contrairement à `VoteScreen`/`AllocatingScreen` ; comportement préexistant, pas une régression du chantier) : `PhaseIndicator` passe à "Étape 6 · Résultats", **le bouton "↻ Revoter" a bien disparu**. Modale des étapes vérifiée : 6 lignes (Distanciel → Résultats), la ligne "Post-débat" cochée.
  - [x] Zéro erreur console sur toute la séquence.
  - [ ] **Non couvert par ce test** (à dérouler avec le mot de passe superadmin quand disponible) : le clic réel sur le bouton `PhaseBar`/`handlePhaseChange` (`debating → post_voting`) et son appel à `force_session_questionnaire` — testé ici uniquement côté effet observé (le questionnaire s'est bien forcé), pas côté déclencheur superadmin. Purge de `reclaim_code` sur `closed` — le membre de test n'en avait pas (inscrit en phase `voting`, pas `pre_voting`), rien à observer. Visiteur non inscrit sur `#session/<code>` en `post_voting` (`SessionRouterScreen`, statut `post_voting_no_member`) — nécessite une deuxième identité anonyme, pas simulée ici (même onglet/navigateur que le participant inscrit).
