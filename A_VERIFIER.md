...
- [ ] **Chantier 65 — `supabase/migrations/20260902_chantier65_register_session_member_reject_draft.sql`** (nouvelle, jamais appliquée) — une séance en phase `draft` ne doit être accessible à personne.
  ...
  *Mis à jour le 2026-09-03 : le chantier 58 (entrée dédiée plus bas, section « Migration SQL en attente d'application ») ferme la partie **colonnes** de ce résidu — `description`/`group_names`/les `doc_*_url` ne sont plus lisibles par un `select()` direct, plus de fuite prématurée de l'analyse PCA. Il ne touche **pas** la policy RLS elle-même : `title`/`join_code`/`phase` d'une séance `draft` restent lisibles par qui devine son `id`, exactement comme laissé ici. Cette partie-là (visibilité des LIGNES, pas des colonnes) reste un résidu ouvert, non traité par le chantier 58 — décision explicite de Jules de ne pas toucher au parcours d'accueil dans ce chantier.*
