...
- [ ] **Écart C2 — asymétrie en phase `allocating`.** Sur une séance en `allocating` : (a) un membre déjà inscrit voit bien le message « les groupes ne peuvent plus être modifiés depuis cet écran » et n'a aucun moyen de se déclarer ; (b) une inscription **neuve** sur le même écran, avec la case « Je suis modérateur » cochée, aboutit bien à `is_moderator = true` et à une affectation immédiate.
- [ ] **Écart C1 — reconquête pré-vote qui perd la déclaration.** En `pre_voting`, saisir un nom déjà inscrit **avec** la case modérateur cochée → le formulaire bascule en reconquête (nom + code) → après succès, vérifier que `session_members.is_moderator` est resté `false`, sans aucun message à l'écran.
