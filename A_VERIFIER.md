...
**Constat** : sur le projet Supabase **dev** (`mnjqrlrrzrycuconlfqb`, cloné le 25/09), deux réglages bloquaient toute session anonyme réelle depuis un navigateur :

1. **Fournisseur d'authentification anonyme désactivé.** `POST /auth/v1/signup` (endpoint utilisé par `signInAnonymously()`) répond `422 anonymous_provider_disabled`. Sur prod, ce même appel fonctionne (c'est ce que fait l'app à chaque nouveau participant). Réglage du dashboard Supabase (Authentication → Sign In / Providers → « Allow anonymous sign-ins ») — **toujours ouvert**, à activer par Jules (aucun outil MCP disponible dans cette session pour le faire à sa place) : `https://supabase.com/dashboard/project/mnjqrlrrzrycuconlfqb/auth/providers`.
2. ~~**Aucune table `public` n'a de GRANT pour `anon`/`authenticated`.**~~ **Corrigé le 2026-09-26**, confirmé par Jules avant application (`supabase/migrations/20260926_fix_dev_grants_tables_sequences.sql`) : `pg_default_acl` (schéma `public`) était entièrement vide sur dev, alors que prod a un `ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public GRANT ALL ON TABLES/SEQUENCES TO anon, authenticated, service_role` — réglage posé une fois par Supabase au bootstrap du projet, jamais écrit dans une migration versionnée, donc invisible à toute introspection du schéma courant. Corrigé : `GRANT ALL` explicite sur les 22 tables + la séquence existantes, et `ALTER DEFAULT PRIVILEGES` pour que toute table future créée par une migration hérite des mêmes droits automatiquement (comme sur prod). Vérifié : `information_schema.role_table_grants` passe de 0 à 308 lignes pour anon/authenticated/service_role sur dev.

**Impact pratique** : le point 2 étant réglé, il ne reste plus que le point 1 (auth anonyme désactivée) pour qu'une vraie session participant fonctionne au navigateur sur dev. Le chantier 131 (voir entrée ci-dessous) n'a pu être vérifié qu'en base (transactions jetables, `auth.uid()` simulé), pas au navigateur — à rejouer une fois le point 1 activé.
...
**Vérifié au navigateur réel le 2026-09-22** (séance QA jetable créée en base puis supprimée ; Jules a saisi lui-même le mot de passe superadmin dans le panneau, la session ne l'a ni demandé ni lu). Points **1, 2, 3, 4 et 5 validés à l'écran** ; le point 3 a révélé une **troisième cause racine** (détection de collision dnd-kit, voir ci-dessous) corrigée dans la foulée et revérifiée. Les 6 scénarios RPC ont par ailleurs été joués en base (transaction jetable).

⚠️ **Découverte de la vérification, à ne pas reperdre** : le `DndContext` de l'onglet Groupes n'avait aucun `collisionDetection`. Sous le `rectIntersection` par défaut, la zone « glisse un participant ici » (`add-moderator-<n>`) était **inatteignable** — imbriquée dans `group-<n>`, elle perdait toujours au calcul d'aire. C'était la cause principale du symptôme de Jules. Corrigé par `pointerWithin` + priorité explicite à `add-moderator-*`. **Toute nouvelle zone de dépôt imbriquée dans une carte de table retombera dans le même piège si elle est ajoutée sans y penser.**

**Parcours binôme joué à l'écran le 2026-09-22** (séance QA jetable : 20 membres, 2 modérateurs, 3 binômes réciproques dont **2 impliquant un modérateur**, puis « Calculer la répartition » et « Appliquer » depuis `AllocationPanel`). Résultat : bandeau **« 2/2 conformes · 🔗 3/3 grappe(s) réunies »**, et à l'écran Table N°1 → modérateur *QB Modo Anne* avec son binôme *QB Lea* 🔗 assis à sa table, Table N°2 → modérateur *QB Modo Karim* avec son binôme *QB Marc* 🔗 à la sienne ; binôme témoin sans modérateur réuni lui aussi. Séance supprimée après coup (aucune ligne QA restante, vérifié).

**Nettoyage fait le 2026-09-22, sur demande explicite de Jules** : les deux `active_moderator_member_id` périmés de la « Séance test de bout en bout » (table 1 → *Jules Bec tel*, en réalité assis en table 2 ; table 2 → *Jules Bec Ordi*, dont le drapeau modérateur avait été retiré depuis) ont été remis à `NULL`. **Aucun modérateur n'a été désigné à sa place** : les deux tables sont « en attente de modérateur », à réattribuer depuis l'onglet Groupes. Contrôle après coup : **0 animateur fantôme sur l'ensemble des séances**.

**Reste à vérifier** : (séance en phase `allocating` ou `debating`, onglet Groupes du superadmin) :
...
6. ✅ **Binôme d'un modérateur qui anime** *(fait — voir ci-dessus)* — déclarer un appairage réciproque entre un participant et une personne flaguée modérateur, puis lancer l'allocation depuis `AllocationPanel`. Attendu : le participant est à la table qu'anime son binôme. Couvert par 3 tests unitaires, **jamais joué à l'écran**.
7. ⬜ **Non-régression de l'allocation sur une population réaliste** ← *seul point encore ouvert* — la réparation par échange peut dégrader l'hétérogénéité ou un seuil d'actifs (c'est le coût assumé de la règle 1, chantier 92). Vérifier sur le tableau de bord de seuils d'`AllocationPanel` que la dégradation reste marginale sur une séance de taille réelle avec plusieurs binômes impliquant des modérateurs.
...
**2. Popup "proposer une assertion" après 10 votes** (`src/screens/VoteScreen.tsx`). Cause confirmée : `nextNudgeAt` repartait toujours de `10` au montage du composant (pas de persistance), donc redéclenchait le popup dès qu'un reload survenait après 10 votes déjà posés. Fix : un `useRef` (`nudgeInitializedRef`) empêche toute réinitialisation après le tout premier appel de `loadVoteData` (le point d'entrée unique de tout chargement de vote — inscription, reclaim, resync Realtime, polling) ; à ce premier appel, `nextNudgeAt` est calculé à partir du nombre de votes déjà posés (`Math.floor(voted / 10) * 10 + 10`) plutôt que fixé à `10`. Les futurs franchissements de seuil pendant la même session (sans reload) continuent de déclencher le nudge normalement, car ils passent par `setMyVotes` dans `handleVote`, pas par `loadVoteData`. **Bouton manuel de repli** : le bouton "✏️ Proposer" déjà présent en permanence dans l'en-tête de l'écran de vote (ligne ~917) ouvre directement `SubmitAssertionModal`, la même destination que le CTA du nudge — pas de nouveau bouton ajouté, celui-ci suffisait déjà. **Non vérifié au navigateur** (demande une séance de vote avec 10+ votes réels et un reload à ce point précis pour confirmer que le popup ne revient plus) — à jouer : voter sur 10 assertions, recharger la page, vérifier que le popup ne réapparaît pas mais que le bouton "Proposer" reste accessible.

**3. Onglet Groupes du superadmin : nombre d'actifs par table** (`src/screens/SuperadminScreen.tsx`, ~ligne 2697). Un badge `"N actif(s)"` a été ajouté à côté du compteur de membres total sur chaque carte de table, en réutilisant `memberProfiles` (déjà calculé, `Map<member_id, AllocationMember>`, porte `is_active`) — aucune nouvelle donnée chargée. Le badge ne s'affiche que si au moins un membre de la table a un profil résolu (sinon `allocInputs` n'est pas encore chargé, ex. avant le premier calcul d'allocation). **Non vérifié au navigateur** : nécessite une séance avec allocation calculée pour avoir des données `is_active` à afficher — à jouer : ouvrir l'onglet Groupes d'une séance en `allocating`/`debating` avec des tables peuplées, vérifier que le badge actifs correspond au nombre réel de participants actifs (croiser avec le badge lettré "a"/"a barré" déjà affiché par membre).

## Chantier 119 (2026-09-21) — Angles morts du code de rappel (inscriptions sans code) — ✅ vérifié en base ET au navigateur réel, ⚠️ deux des trois chemins non rejoués à l'écran

> ⚠️ Numéroté 117, puis 118, au fil de deux rebases — deux autres sessions ont pris ces numéros le même jour (117 : « Vue Groupes », ci-dessous ; 118 : refonte du modérateur physique en session_member, encore en file d'attente, `docs/chantiers-a-faire.md` § 118, qui recoupe directement ce chantier-ci sur `claim_table_as_moderator` — voir l'amendement en tête de sa spec). Définitivement 119.

Demandé par Jules après avoir constaté à l'écran qu'un débatteur rejoignant une séance en `debating` par un code de table n'avait jamais reçu son code de rappel. Audit exhaustif en base (`select proname from pg_proc ... where prosrc ilike '%insert into session_members%'` — les 6 fonctions qui créent une ligne `session_members`) : trois d'entre elles ne généraient aucun `reclaim_code` — `sync_table_assignment` (appelée par `join_table`/`switch_table`/`create_table`, donc par tout « rejoindre une table par son code »), `claim_table_as_moderator` (chantier 68, n'appelait même pas `sync_table_assignment` : aucune ligne `session_members`/`table_assignments` créée pour ce modérateur de rattrapage) et `add_offline_participant` (outil modérateur « ajouter une personne sans téléphone », identité fictive `gen_random_uuid()` jamais reliée à un vrai appareil). Détail complet et fix retenu dans `docs/chantiers-a-faire.md` § 119.

`sync_table_assignment` change de type de retour (`void` → `jsonb`) pour remonter le nouveau code ; `DROP FUNCTION IF EXISTS` puis recréation, ACL internes (chantier 102) reconstituées à l'identique — **piège rencontré et documenté dans la migration** : le simple `DROP`+`CREATE` a déclenché les privilèges par défaut de Supabase (regrant explicite `EXECUTE` à `anon`/`authenticated` sur toute fonction recréée), qu'un `REVOKE ... FROM PUBLIC` seul ne retire pas puisque ce sont des grants explicites et non hérités — repéré en relisant `pg_proc.proacl` après application, corrigé en nommant `anon, authenticated` explicitement dans le `REVOKE`.

**Vérifié en base le 2026-09-21** (transaction jetable `BEGIN…ROLLBACK`, quatre identités fictives, tout annulé) :
- `join_table` (nouvelle identité) → `new_reclaim_code` présent dans le résultat, `session_members.reclaim_code_hash` non nul.
- `claim_table_as_moderator` (Code Ecclesia de test posé et restauré dans la même transaction) → `new_reclaim_code` présent, `session_members` créé avec le bon `user_id` (l'appelant, pas le superadmin).
- `add_offline_participant` → `new_reclaim_code` présent, `session_members.user_id` bien une identité fictive (`gen_random_uuid()`, comportement voulu — la personne réclamera son profil plus tard via `confirm_attendance` pseudo+code depuis son propre appareil).
- `switch_table` par un membre **déjà inscrit** → `new_reclaim_code` **absent** du résultat, confirmant qu'aucun nouveau code n'est généré pour un changement de table en cours de séance (pas de régression sur le cas nominal).

**Vérifié au navigateur réel le 2026-09-21** (dev server sur ce worktree, séance QA jetable `73c1bd08…`/table `Q117TB` créée puis intégralement purgée en base après coup, `0` ligne restante confirmé) : depuis `#table/Q117TB` (lien d'ami, `JoinTableForm`/`JoinTableScreen`), saisie du nom → écran « Note ton code de rappel » (`ReclaimCodeDisplay`, bouton « Rejoindre la table → » contextualisé) → code affiché (`8206`) → clic → arrivée dans `ParticipantView` de la table. Code affiché à l'écran confirmé identique au hash stocké en base (`crypt('8206', reclaim_code_hash) = reclaim_code_hash` → `true`).

**Non vérifié à l'écran** : les chemins `claim_table_as_moderator` (case « Je suis modérateur de cette table » dans `JoinTableForm`/`TableAssignmentCard`) et `add_offline_participant` (modale « Ajouter une personne sans téléphone », `ModeratorToolsButton.tsx`) — couverts par la vérification en base ci-dessus, mais l'affichage réel du code à l'écran pour ces deux cas n'a pas été rejoué (le premier demande un vrai Code Ecclesia, non disponible pour cette session ; le second demande une séance déjà en `debating` avec un modérateur physique déjà assis, scénario plus long à mettre en scène qu'un simple rejoindre-par-code).

**Hors périmètre, découvert en cours de route** : `App.tsx` (restauration après renouvellement du jeton anonyme) appelle aussi `join_table`, mais avec le **nouveau** `auth.uid()` — `sync_table_assignment` ne retrouve pas l'ancienne ligne `session_members` (dont le `user_id` est resté l'ancien), tente un nouvel `INSERT`, qui échoue en silence sur la contrainte d'unicité `(session_id, pseudo)` (capturé par le `EXCEPTION WHEN OTHERS` qui avale l'erreur). Le renouvellement de jeton semble donc pouvoir désynchroniser durablement `session_members.user_id` de l'identité réelle de l'appareil, indépendamment du code de rappel — à vérifier et traiter séparément si confirmé, pas touché par ce chantier.
...
**Vérifié en base le 2026-09-21** (séance de test `TEST116` créée puis purgée) : après régénération, `crypt('<ancien code>', reclaim_code_hash) = reclaim_code_hash` renvoie `false`, `crypt('<nouveau code>', reclaim_code_hash) = reclaim_code_hash` renvoie `true`. ✅

**Vérifié au navigateur réel le 2026-09-21** (dev server sur ce worktree, `.env` copié depuis `chantier-112-5b0a4d`, séance jetable `TEST116` en `pre_voting`) : inscription (code initial noté à l'écran), passage à l'étape vote, ouverture d'Outils → « Changer mon nom / code » ouvre bien `RenamePseudoModal` avec le nouveau bloc, clic « Faire réapparaître mon code » → nouveau code affiché immédiatement, cohérent avec la vérification en base ci-dessus. Séance de test supprimée après coup (`DELETE FROM sessions ... CASCADE`, `session_members` vérifié vide ensuite).

**Non vérifié à l'écran** : le cas `reclaim_code_hash IS NULL` (membre jamais inscrit avec un code, ou séance déjà `closed` — chantier 49 purge la colonne à la clôture). Dans ce cas, `regenerate_reclaim_code_self` réussit quand même et **émet un nouveau code** (comportement voulu : sur une séance encore ouverte mais où le code avait déjà été utilisé/perdu autrement, régénérer reste utile) ; sur une séance `closed`, la fonction n'a pas de garde de phase explicite — à confirmer si un usage post-clôture doit être bloqué comme le sont `confirm_attendance`/`reclaim_prevoting_member`.

**Même session, changement mineur non numéroté** : remplacement de l'icône générique « deux personnes » par le logo Ecclesia fourni par Jules dans l'en-tête du menu principal (`EntryScreen.tsx`, `src/assets/ecclesia-logo.webp`). Vérifié visuellement au navigateur, capture à l'écran de l'en-tête conforme au logo fourni.

---

## Complétion chantier 93 (2026-09-21) — bouton clef déplacé de la liste des participants vers « Outils Modo » → « Code participant », avec confirmation avant régénération — non vérifié à l'écran

Demande de Jules en chat (pas dans la file d'attente) : le bouton 🔑 par ligne dans `ParticipantsTable.tsx` (chantier 93) est retiré et remplacé par une entrée « Code participant » dans le menu « Outils Modo » (section Table, visible seulement si `table.session_id`, comme avant). Cette entrée ouvre `ParticipantCodesModal.tsx` (nouveau composant) : liste des participants avec un bouton clef chacun. Un clic ouvre désormais `ConfirmModal` (« Remplacer le code de *pseudo* ? ») avant d'appeler `regenerateReclaimCodeModerator` — auparavant l'appel partait directement au clic. RPC et modale d'affichage du nouveau code (`newCode`) inchangées, seulement déplacées dans le nouveau composant.

**Vérifié** : `tsc --noEmit` propre après le déplacement (imports/props nettoyés dans `ParticipantsTable.tsx`, `showToast` restauré pour le toast d'exclusion qui restait dans ce fichier).

**Non vérifié à l'écran** : impossible de dérouler ce chantier au navigateur dans cette session — la seule séance disponible sur le projet Supabase relié à ce worktree (`.env` copié depuis la racine) est une séance réelle en cours (« Réunion apprentissage modération 21/09 », débat en cours), et créer une séance/table jetable pour un vrai modérateur (`is_table_moderator`) demande une ligne `session_members` + `table_assignments` + un utilisateur authentifié, non mis en scène ici faute de temps. À vérifier à la main : ouverture du panneau « Code participant » depuis Outils Modo, présence du bouton clef par participant, apparition de la confirmation avant régénération, et affichage correct du nouveau code après confirmation.

## Chantier 115 (2026-09-21) — « Mes binômes » retiré du débat, remplacé par « Changer de table » — ✅ vérifié en base ET au navigateur réel

Dicté par Jules en chat, pas dans la file d'attente. Constat de départ : le rattachement automatique par binôme (chantier 92, phases `allocating`/`debating`) ne fonctionnait que pour un retardataire sans table — pour qui était déjà assis, le garde `NOT EXISTS (... table_assignments ...)` bloquait silencieusement tout déplacement, rendant le bouton « Mes binômes » inutile une fois en débat. `set_my_pairings` resserré à la phase `voting` uniquement ; le bloc de rattachement automatique supprimé (`placed_table_number` retourne toujours `NULL` désormais). Nouveau composant `ChangeTableModal.tsx` (code de table + bouton « Rejoindre », calqué sur le bloc déjà existant dans `TableChangeModal.tsx` du chantier 95) affiché dans `ParticipantToolsButton` (Outils, débat uniquement — pas en `allocating`, où aucune table n'existe encore). « Mes binômes » retiré de `ParticipantToolsButton` et d'`AllocatingScreen` ; renommé « Être avec un ami (facultatif) » et conservé dans `VoteToolsPanel`/`OnboardingForm`/`PairingModal` (resserré à `voting`), seul contexte où c'est encore une vraie préférence pour l'algorithme d'allocation.

**Comparaison avant migration** (`pg_get_functiondef` sur `set_my_pairings` en base, avant d'écrire la migration) : corps identique au dernier fichier de migration du dépôt (chantier 92) — aucune dérive détectée, migration appliquée directement.

**Vérifié en base le 2026-09-21** (appel réel, pas de transaction jetable ici — juste une lecture d'erreur) : `set_my_pairings` appelé sur une séance `debating` échoue avec `Les binômes ne peuvent être déclarés qu'en phase de vote présentiel` — la restriction de phase est bien active. ✅

**Vérifié au navigateur réel le 2026-09-21** (dev server local sur ce worktree, `.env` copié depuis le dossier racine, séance et deux tables jetables créées/purgées en base — `sessions.title = '🧪 QA chantier 113 (jetable)'` (nom littéral posé en base avant la découverte de la collision de numérotation ci-dessous, sans conséquence), join_codes `QA113A`/`QA113B`) :
1. Participant posé via `localStorage.ecclesia_table` + une ligne `participants` sur `QA113A`, séance en `debating`. Ouverture du panneau Outils : « Mes binômes » a bien disparu, « 🔀 Changer de table » apparaît à sa place.
2. Clic → modale « Changer de table » (champ code + bouton « Rejoindre »). Saisie de `qa113b` (uppercase automatique confirmé), clic « Rejoindre » → `join_table` appelé, page rechargée, le header affiche désormais `QA113B`.
3. Confirmé en base après coup : plus aucune ligne `participants` sur `QA113A`, une ligne sur `QA113B` ; `table_assignments.table_number = 2`. `leave_other_session_tables` (appelé par `join_table`) a donc bien nettoyé l'ancienne place.
4. Nettoyage : `DELETE FROM sessions WHERE id = …` (cascade), `tables` avec ces `join_code` confirmées absentes après coup (0 ligne restante).

**Non vérifié à l'écran** : le bouton « Être avec un ami (facultatif) » conservé côté `VoteScreen`/`OnboardingForm` — seul le libellé change (texte du bouton, titre de la modale, eyebrow), la logique et le composant `PairingModal` ne sont pas modifiés autrement que par le resserrement de phase déjà couvert par la vérification SQL ci-dessus. Risque de régression jugé négligeable mais pas formellement observé à l'écran (pas de mot de passe superadmin nécessaire ici, mais scénario `voting` avec deux membres à faire cohabiter — pas mis en scène pour ce chantier).

## Chantier 107 (2026-09-21) — `claim_moderator_status` ne doit jamais déplacer quelqu'un déjà assis — ✅ vérifié en base ET au navigateur réel avec le vrai Code Ecclesia

`claim_moderator_status` : la branche `ELSE` (recherche + réécriture de `table_assignments` vers la première table animée sans modérateur) ne s'exécute désormais que si l'appelant n'a **aucune** ligne `table_assignments` **et** que la séance n'est **pas** en phase `allocating`. Dans tous les autres cas (déjà assis ailleurs, ou phase `allocating`), la fonction ne pose plus que `session_members.is_moderator = true`. La conversion en place d'une table `leaderless` où l'appelant est déjà assis (chantier 64) est inchangée.

**Vérification SQL directe le 2026-09-21** (rôle service de l'outil Supabase MCP, `auth.uid()` simulé par `set_config('request.jwt.claim.sub', …)`, tout dans une transaction explicite `BEGIN … ROLLBACK` — `creation_code_hash` temporairement remplacé par un hash de test connu **à l'intérieur de la même transaction**, donc invisible de l'extérieur, et confirmé restauré après coup) :
1. Séance jetable en phase `debating`, deux tables non-`leaderless` ; un membre modérateur déjà assis sur la table 2, un second membre (le testeur) assis sur la table 1, pas encore modérateur. Le testeur appelle `claim_moderator_status` : `session_members.is_moderator` passe à `true`, et il **reste sur la table 1** (`table_assignments.table_number = 1` inchangé) — avant ce chantier, il aurait été réaffecté à la table 1 de toute façon ici (déjà seule libre), donc ce cas ne distinguait pas ancien/nouveau comportement à lui seul, mais confirme l'absence de régression. ✅
2. Séance jetable en phase `allocating`, une table non-`leaderless` sans aucun modérateur, aucun membre inscrit. Un nouveau membre s'inscrit et se déclare modérateur dans le même appel (`p_pseudo` renseigné, création de la ligne `session_members`) : le flag est bien posé (`is_moderator = true`) mais **aucune ligne `table_assignments` n'est créée** (`count = 0`) — avant ce chantier, il aurait été placé d'office sur l'unique table libre. C'est le cas que ce chantier devait fermer. ✅
3. Transaction annulée (`ROLLBACK`) dans les deux cas : séances/tables/membres jetables absents après coup, `creation_code_hash` revenu à sa valeur d'avant test — vérifié par une requête séparée après coup. ✅

**Vérifié au navigateur le 2026-09-21** (dev server local sur ce worktree, `.env` copié depuis le dossier racine, `npm install`, deux séances jetables créées directement en base — `sessions.title` préfixés `🧪 QA107a`/`🧪 QA107b`, purgées par `DELETE ... CASCADE` après coup — **vrai Code Ecclesia fourni par Jules pour ce test uniquement**, jamais stocké dans ce dépôt ni dans une mémoire persistante) :
1. **Cas central de l'audit, celui qui motivait le chantier** — séance jetable en phase `voting`, deux tables non-`leaderless` (table 1 libre sans modérateur, table 2 avec le testeur déjà assis dessus via `table_assignments` posé en SQL, pas encore modérateur). Le testeur déclare via le vrai formulaire `VotingEntryForm` (reconquête nom + code de rappel + case « Je suis modérateur » + vrai Code Ecclesia), en repartant d'une session anonyme fraîche (jeton `sb-…-auth-token` effacé avant reconquête, pour que `confirm_attendance` rebranche bien `auth.uid()` sur ce membre). Résultat en base après soumission : `is_moderator = true`, **toujours assis sur la table 2** (`table_assignments.table_number = 2` inchangé) — avec l'ancien code, la table 1 (plus petit numéro sans modérateur) aurait été choisie et l'aurait déplacé. Aucun message d'échec affiché côté UI (« Bienvenue … Tes votes ont bien été récupérés », sans le bandeau ambre d'échec de déclaration). ✅ Comportement réel confirmé, pas seulement simulé en SQL.
2. **Déclaration pendant `allocating`** — séance jetable en phase `allocating`, une table non-`leaderless` sans aucun membre inscrit. Inscription neuve via le même formulaire (case cochée + vrai Code Ecclesia dès la première inscription, pas de code de rappel). Résultat en base : `is_moderator = true`, **aucune ligne `table_assignments` créée** (`count = 0`) — avec l'ancien code, il aurait été placé d'office sur l'unique table libre. Parcours poursuivi jusqu'à l'onboarding sans blocage, confirmant qu'aucune exception n'a été levée côté RPC. ✅
3. Nettoyage : les deux séances jetables et leur contenu (tables, membres, entry_responses) supprimés par cascade après le test — vérifié par requête de comptage après coup (0 ligne restante). Serveur de dev arrêté.

**Non testé par cette vérification navigateur, hors périmètre de ce chantier (SQL only)** :
...
**Vérifié en base le 2026-09-21** (deux transactions jetables `BEGIN…ROLLBACK`, `assign_least_filled_table` appelée avec de fausses identités `auth.uid()` simulées via `set_config('request.jwt.claim.sub', …)`) :
1. Deux tables animées vides d'une même séance `debating` : `assign_least_filled_table` choisit la table de plus petit `table_number` (tie-break déterministe, §6). ✅
2. Une table occupée par un premier participant (posé via un vrai `join_table` juste avant, même transaction) : le retardataire suivant atterrit bien sur l'**autre** table, la moins remplie — vérifié par le `join_code` retourné dans le résultat jsonb. `session_members` créé avec `reclaim_code_hash` non nul (`new_reclaim_code` retourné en clair dans la réponse, une seule fois — comme `register_session_member`) ; `table_assignments` correct (bon `table_id`, bon `table_number`). ✅
3. Les deux transactions ont été annulées par une `RAISE EXCEPTION` volontaire après capture des résultats — aucune trace laissée en base (vérifié par requête de comptage séparée après coup).

**Vérifié au navigateur réel le 2026-09-21** (dev server local sur ce worktree, `.env` copié depuis le dossier racine, séance et deux tables jetables créées/purgées en base pour ce test — `sessions.title` préfixé `🧪 QA111`, join_codes `QA111NAV`/`QANAVA`/`QANAVB`) :
1. Navigation vers `#session/QA111NAV` sur une séance en phase `debating`, sans avoir jamais été inscrit à la séance ni auth anonyme préexistante côté client → écran « Débat en cours » avec le nouveau bouton « Assignez-moi une table » affiché au-dessus du formulaire de code (capture texte prise via `get_page_text`).
2. Saisie d'un nom, clic sur le bouton → placement immédiat dans `ParticipantView` de la table `QANAVA` (la moins remplie, seule occupée à ce stade), sans jamais taper de code à 6 caractères.
3. Confirmé en base après coup : `session_members.pseudo` correct, `attending_in_person = true`, `reclaim_code_hash` non nul, `table_assignments.table_number = 1` pointant vers `QANAVA`.
4. Nettoyage : `DELETE FROM sessions WHERE id = …` (cascade), tables associées confirmées absentes après coup (0 ligne restante).

**Non rejoué à l'écran** : le second point d'entrée du bouton (`TableAssignmentCard.tsx`/`AllocatingScreen.tsx`, pour un membre **déjà inscrit** — voté ou rejoint pendant l'allocation — mais sans affectation de table, le cul-de-sac du chantier 62). Il partage exactement le même composant (`TableAssignmentCard`, prop `onAssignLeastFilled`) et la même RPC que le chemin vérifié ci-dessus ; non rejoué séparément au navigateur faute de pouvoir simuler facilement deux identités anonymes distinctes dans le même onglet (l'auth anonyme Supabase persiste par origine dans `localStorage` — un second onglet du même navigateur hérite de la même identité que le premier, donc `SessionRouterScreen` route directement vers `#vote/` au lieu de rejouer le scénario "jamais inscrit").
...
1. Séance jetable en `allocating`, deux tables animées : table 1 avec un modérateur déjà en exercice (`active_moderator_member_id` posé + `table_assignments`), table 2 sans personne. Deux modérateurs supplémentaires `is_moderator=true` sans aucune table. Dry-run : `placements = [{Modo En Attente 1 → table 2}]`, `unplaced_moderators = [Modo En Attente 2]` (plus de modérateurs en attente que de tables libres), `tables_without_moderator = []`. ✅ — comportement conforme à l'arbitrage 2 (le surplus reste participant, aucun traitement particulier).
2. `apply=true` sur le même scénario : `table_assignments` du "Modo En Attente 1" bien réécrit sur la table 2 (`table_number=2`, `table_id` de la table 2) ; celui du modérateur déjà en exercice (table 1) inchangé. ✅
3. Dry-run rejoué juste après l'apply : `placements = []`, "Modo En Attente 1" n'apparaît plus (il est maintenant en exercice sur la table 2) — confirme l'idempotence et que le dry-run et l'apply appliquent exactement le même calcul. ✅
4. Transaction annulée (`ROLLBACK`) : séance/tables/membres jetables absents après coup (vérifié par requête séparée), et la fonction déployée en base a bien retrouvé sa garde `check_superadmin_password` (un appel avec un mauvais mot de passe juste après échoue normalement). ✅

**Non vérifié à l'écran** (pas de mot de passe superadmin pour cette session, comme au chantier 106) :
- Le récapitulatif affiché dans la modale « Ouvrir le débat » (liste des noms + tables visées, avertissements en ambre) n'a été vu qu'en lisant le JSX, pas rendu dans un vrai navigateur avec de vraies données.
- Le cas « aucun modérateur en attente » (bouton direct, pas de dry-run à afficher autre que « Aucun modérateur en attente à placer »).
- Que `ModeratorView` s'affiche bien, après le clic réel, chez le modérateur nouvellement placé (dépend de `is_table_moderator`, chantier 106, déjà vérifié séparément côté SQL et navigateur pour ce mécanisme).
...
**Vérifié au navigateur le 2026-09-21** (séance QA jetable créée directement en base — `sessions.title = '🧪 QA chantier 112 (jetable)'`, purgée après coup ; pas de mot de passe superadmin/Code Ecclesia utilisé, le modérateur a été posé directement via `localStorage.ecclesia_table` + une ligne `participants`) :
1. Table en phase `debating`, `questionnaire_forced_at = null` → `ModeratorView` normal, pas d'overlay. ✅
2. `sessions.phase → 'post_voting'`, `questionnaire_forced_at → now()` → après le polling 5s (`TableContext.load`), pas d'overlay ; "Outils Modo" cliquable ; "Questionnaire post-débat" ouvre bien le modal (notation, champs texte, bouton Envoyer fonctionnels). ✅
3. Contre-épreuve : `questionnaire_forced_at → null` (phase restant `post_voting`) → l'overlay "La séance est terminée" apparaît bien (confirme que le polling avait bien fait passer `session.phase` côté client, et que c'est la garde qui le masquait à l'étape 2, pas une session.phase pas encore à jour). ✅
4. `questionnaire_forced_at → now() - 2h` (fenêtre expirée) → overlay réapparaît. ✅
5. `sessions.phase → 'closed'`, `questionnaire_forced_at → now()` (forçage encore actif au moment de la clôture) → overlay toujours masqué, "Outils Modo" toujours accessible — comportement volontairement identique à `post_voting`, cohérent avec la garde `!forcedQOpen` de `ParticipantView` qui ne distingue pas non plus les deux phases. ✅

**Non couvert par ce test** (à surveiller si un doute apparaît en production) : le forçage déclenché par la vraie RPC `force_session_questionnaire` (ici `questionnaire_forced_at` a été posé à la main en SQL, pas via la RPC réelle) ; le comportement quand `showModWelcome` (modale de bienvenue modérateur) et l'overlay sont tous deux candidats à l'affichage — observé que la modale de bienvenue passe devant sans conflit apparent, mais les deux ne sont pas mutuellement exclusifs dans le code et ça n'a pas été creusé plus loin, hors périmètre du chantier 112.

## Chantier 110 (2026-09-21) — Bouton « Je suis le modérateur de cette table » dans les Outils + filet d'identité — ✅ vérifié en base, ⚠️ succès RPC non rejoué à l'écran (pas de vrai Code Ecclesia pour cette session)

Nouvelle RPC `reclaim_table_as_moderator(table_id, creation_code)` : Code Ecclesia requis, exige que l'appelant soit déjà assis à la table (`is_table_participant`), puis transfère `tables.created_by` **et** `active_moderator_member_id` d'un coup, **sans COALESCE** — contrairement à tous les autres chemins du chantier 106, celui-ci déloge volontairement un titulaire déjà en exercice (transfert/passation de main assumé, tranché explicitement par Jules comme le cœur du chantier). Nouveau bouton dans `ParticipantToolsButton` (« Je suis le modérateur de cette table », masqué si déjà `isModerator`), nouvelle modale `ReclaimTableModeratorModal`. `App.tsx` : l'ex-calcul `isMod = r.created_by === userId` (toujours faux, anti-pattern CLAUDE.md) est retiré ; après un `join_table` de reconquête, `isModerator` part systématiquement à `false` — ce bouton est désormais le seul filet de rattrapage. `TableContext.tsx` : toast informatif pour l'ancien titulaire quand il est délogé en direct via le canal Realtime `table:<id>` déjà ouvert.

**Vérification SQL directe le 2026-09-21** (outil Supabase MCP, tout dans des transactions explicites `BEGIN … ROLLBACK`, `creation_code_hash` temporairement remplacé par un hash de test connu **à l'intérieur de la transaction**, confirmé restauré après coup — deux transactions séparées, la première volontairement avortée par l'exception attendue) :
1. Un appelant qui n'est assis nulle part sur la table visée est refusé (`Tu dois être assis à cette table pour en reprendre l'animation`), y compris avec le bon Code Ecclesia. ✅ (transaction automatiquement annulée par l'exception, rien persisté)
2. Deux participants assis à la même table autonome (hors séance), le premier déjà modérateur (`is_table_moderator` = `true` avant test). Le second, déjà assis, appelle `reclaim_table_as_moderator` avec le bon Code Ecclesia malgré la présence du premier modérateur : succès. Après l'appel, `is_table_moderator` bascule à `true` pour le second et `false` pour le premier ; `tables.created_by` transféré au second. ✅ — **le cas central du chantier**, refusé par `claim_table_as_moderator` (chantier 68/72) et réussi ici volontairement.
3. Transactions annulées (`ROLLBACK`) dans les deux cas : aucune table/participant jetable restant après coup (vérifié par requête séparée), `creation_code_hash` revenu à sa valeur d'avant test.

**Vérifié au navigateur le 2026-09-21** (dev server local sur ce worktree, `.env` copié depuis le dossier racine, table QA jetable créée directement en base — `tables.join_code = 'QA110X'`, un participant tiers `created_by` fictif comme "modérateur en place", le navigateur de test posé comme second participant non-modérateur via `localStorage.ecclesia_table` + une ligne `participants`, purgés par cascade après coup — 0 ligne restante vérifié) :
1. `ParticipantView` s'affiche pour le testeur (pas encore modérateur), "Outils" → le bouton « Je suis le modérateur de cette table » apparaît bien (masqué à raison, la garde `!isModerator` étant vraie). ✅
2. Clic sur le bouton → modale « Reprendre l'animation de cette table » s'ouvre avec le texte attendu (mention explicite que l'ancien titulaire bascule en vue participant sans perdre son statut). ✅
3. Soumission avec un Code Ecclesia **volontairement invalide** (pas de vrai Code Ecclesia disponible pour cette session) → appel réseau réel à `reclaim_table_as_moderator`, réponse serveur affichée telle quelle : « Code Ecclesia incorrect ». Confirme que le bouton, la modale et le câblage réseau fonctionnent de bout en bout jusqu'à la RPC réelle. ✅
4. Nettoyage : table QA et son participant supprimés par cascade après le test (vérifié par requête de comptage, 0 ligne restante), `localStorage.ecclesia_table` effacé côté navigateur, serveur de dev arrêté.
...
**Validé par Jules le 2026-09-16 — rapport : [`docs/chantier-91-comparatif-allocation.md`](./docs/chantier-91-comparatif-allocation.md)**. Question ouverte : les modérateurs en surplus comptent-ils dans la limite de 30 (salle de 30 personnes / 6 actifs / 3 modérateurs → table de 32) ?
...
**Ce qui a été fait** :
- Infra toasts : `src/context/ToastContext.tsx` (`ToastProvider`/`useToast`), monté à la racine dans `main.tsx`. Pile en bas d'écran, auto-dismiss 3 s, 3 variantes (succès/erreur/info). Animation `toast-in` ajoutée à `tailwind.config.js`.
- Usages branchés : `ParticipantsTable.tsx` (toast succès après exclusion d'un participant), `TableContext.tsx` (toast erreur quand la table courante est supprimée pendant qu'on y est — chargement initial et événement Realtime DELETE), `App.tsx` (toast info quand la table restaurée depuis `localStorage` au démarrage n'existe plus).
- Page 404 : `src/screens/NotFoundScreen.tsx`, réutilisant le style déjà en place dans `SessionRouterScreen`/`VoteScreen` pour les états "introuvable" plutôt que d'introduire un style concurrent. Branchée dans `App.tsx` pour tout hash non vide qui ne correspond à aucune route connue (`#superadmin`, `#collab/`, `#session/`, `#vote/`, `#results/`, `#table/`) — avant, ce cas retombait silencieusement sur l'écran d'accueil sans explication.
- QR code superadmin : `ShareLinkBanner` (composant interne à `SuperadminScreen.tsx`, onglet Préparation) a maintenant un bouton "QR code" à côté de "Copier", réutilisant `QrCodeModal` (déjà utilisé côté modérateur/participant) sur le lien `#session/<join_code>`. Remplace le recours à un site externe.

**Volontairement pas touché** : aucun fichier de la liste gelée jusqu'au 10/09 (`docs/registre-merges-en-attente.md` §1) — en particulier ni `VoteScreen.tsx`, ni `ReclaimCodeDisplay.tsx` (utilisé par le flux de vote), pour ne rien risquer sur la séance réelle du 10/09. Les autres points de copie silencieuse identifiés (`ReclaimCodeDisplay`, `CollabDocScreen`) n'ont pas reçu de toast pour cette même raison de prudence — à reprendre après le 10/09 si souhaité. La persistance de la pause après rechargement (item annexe B) s'est révélée **déjà implémentée** (`ModeratorView.tsx`, clé `localStorage` par table, avec revalidation à la reprise) — rien à faire.

**Vérifié au navigateur le 2026-09-08 (chantier 88c)** — jeton de serveur de dev pris (aucun autre serveur actif à ce moment), `.env` et `.claude/launch.json` ajoutés à ce worktree (absents jusque-là). Séance réelle en cours vue sur l'accueil (« Quel rôle pour l'IA dans notre société ? », vote présentiel) — **non touchée** : une table jetable dédiée (`TEST88`, standalone, `leaderless`, 4 participants fictifs) a été créée et peuplée par SQL direct (MCP Supabase) plutôt que via l'accueil, précisément pour ne pas interagir avec cette séance réelle. Table et notes de test **purgées après coup** (`DELETE FROM tables WHERE id = ...`, cascade sur `participants`/`queue_entries`/`speaking_turns`).
1. Hash inconnu (`#nimportequoi`) → page 404 affichée, "← Retour à l'accueil" fonctionne, zéro erreur console. **Confirmé.**
2. D1 (règles à l'entrée) → modale "Bienvenue" puis, après clic "Lire les règles de débat Ecclesia →", modale séparée "Règles d'Ecclesia lors des débats" avec bouton unique "J'ai lu →" ; aucune des deux ne réapparaît après reload. **Confirmé** — ordre Bienvenue→Règles (chantier 40), différent de l'ordre Règles→Bienvenue décrit dans `PARCOURS_VERIFICATION.md` (22/07), changement assumé et documenté à l'endroit du code, pas une régression.
...
6. QR code superadmin (item 88b) — **confirmé le 2026-09-15 par Jules lui-même**, en local (serveur de dev sur cette machine, base de production réelle) : bouton visible dans `ShareLinkBanner` (onglet Préparation), séance « Devons nous, et comment, réguler les réseaux sociaux ? ». Aucune anomalie remontée.
...
- [x] **2026-09-08 — Chantier 8 (rattrapage) — Fix DnD : l'entrée déposée n'arrive plus en dernier (A2)** *(validé au navigateur — chantier 88c)*
  ...
  **Vérifié au navigateur le 2026-09-08** (table jetable `TEST88`, créée/purgée par SQL direct — voir en tête de section « Chantier 88a/88b »). Reorder **intra-file** (glisser un participant déjà en file vers une autre position) : confirmé exact — un participant en position 3 déplacé sur la ligne 1 atterrit bien en position 1, les autres décalent proprement. Drag **participant → file** (ajout d'un nouveau) : le comportement dépend de la moitié de ligne visée (au-dessus/en-dessous du point médian) plutôt que d'un "toujours en dernier" — testé deux fois avec des résultats différant selon le point de dépose exact, jamais un append systématique en fin de file. Zéro erreur console sur l'ensemble de la séquence. Le bug original (toujours en dernier, quel que soit le point de dépose) n'est pas reproduit.
...
- [x] **2026-09-06 — Chantier 59 — `supabase/migrations/20260906_chantier59_realtime_canaux_prives.sql`** *(validé le 2026-09-16 — F6 fermé, « Allow public access » désactivé par Jules)* (appliquée le 2026-09-07) — canaux Realtime privés, F6 à la racine
  ...
  **Mise à jour du 2026-09-16 (nouvelle session Claude Code, avec Jules en direct)** — recette rejouée en profondeur, avec **deux vraies identités distinctes** cette fois (Browser pane = Testeur A, vrai Chrome de Jules piloté via Claude in Chrome = Testeur B, deux `auth.uid()` différents, confirmé) :

  - **Scénario A (débat, table `leaderless` de test `T59TEST2`)** : jointure des deux identités, `2 participants` affichés sans reload. Testeur B prend la parole (`claimFloor`, leaderless) → **propagation confirmée** côté Testeur A (réordonnancement de la liste des présents, sans reload). Aucune `CHANNEL_ERROR` sur les deux clients.
  - **Frontière d'autorisation vérifiée en direct** (console, vraie session JWT de Testeur A) : canal de sa propre table → `SUBSCRIBED` ; canal d'une autre table où il n'est pas assis → `CHANNEL_ERROR`/Unauthorized ; client complètement anonyme sans JWT → `CHANNEL_ERROR`/Unauthorized ; le **même topic en mode public** (sans `private:true`) → `SUBSCRIBED` quand même (confirme que F6 reste ouvert tant que « Allow public access » n'est pas désactivé — comportement attendu à ce stade).
  - **Scénarios C, D, E, F, G — les 8 topics de la carte d'autorisation testés individuellement**, avec deux vrais membres insérés dans la séance de test `🧪 Progression complète (draft→clôture)` (`FADE01`, phase `allocating`, session_id `3d07b4e4-9a31-4771-bd0d-8800c8809630`) :
    - `vote:<session_id>` → `SUBSCRIBED` (Testeur B a proposé une assertion réelle via l'UI — « TEST CHANTIER 59 — propagation live » — reçue côté serveur, approuvée automatiquement en `moderation_policy=open`, supprimée après coup)
    - `allocating:<session_id>` → `SUBSCRIBED`
    - `collab:<session_id>` → `SUBSCRIBED`
    - `session-member:<member_id>` → `SUBSCRIBED` pour son propre membre
    - `postvote:<session_id>:<member_id>` → `SUBSCRIBED` pour son propre membre, `CHANNEL_ERROR` pour le membre de Testeur B (frontière vérifiée dans les deux sens)
    - `table:<table_id>` / `session-member-status:<table_id>` déjà couverts par le scénario A
    - **Piège rencontré et résolu** : les premiers essais sur `allocating:`/`vote:` avec un client injecté en console renvoyaient `CHANNEL_ERROR` alors que l'app elle-même n'avait jamais loggé d'erreur — cause identifiée : plusieurs clients Supabase-js empilés sur la **même clé `localStorage`** (`Multiple GoTrueClient instances detected`, warning explicite en console), qui se marchent dessus sur le rafraîchissement de session. Résolu en isolant chaque client de test sur sa propre `storageKey` (`auth.setSession()` avec les tokens copiés, storage dédié) — **pas une régression de chantier 59**, une limite de la méthode de test à connaître pour la prochaine session qui voudrait scripter ce genre de vérification en console.
...
## Chantiers 87 + 101 — Audit du parcours participant & modérateur (2026-09-20)

**Aucun code n'a été modifié par ce chantier** : il n'y a donc rien à vérifier au sens habituel (pas de régression possible). Ce qui suit est la liste des **constats de l'audit qu'il faudrait confirmer à l'écran** avant d'ouvrir les chantiers correctifs — chacun est une hypothèse lue dans le code, pas une observation en séance. Document : [`docs/2026-09-20-audit-chantier87-101-parcours-participant.md`](./docs/2026-09-20-audit-chantier87-101-parcours-participant.md).

- [ ] **Problème A — deux écrans modérateurs sur une même table.** Séance de test, deux membres marqués `session_members.is_moderator = true` et affectés à la **même** table non `leaderless` (le plus simple : `set_member_moderator` sur deux membres d'un même groupe depuis le superadmin). Attendu par l'audit : les deux voient `ModeratorView`, et les deux peuvent réellement donner la parole (`grant_floor` accepté pour les deux). Si c'est confirmé, P1 est justifié.
- [ ] **Problème B — déplacement silencieux par `claim_moderator_status`.** Un membre déjà assis à une table **animée** ouvre « Outils → Me déclarer modérateur » avec le Code Ecclesia, alors qu'il existe ailleurs dans la séance **au moins une** table animée sans modérateur. Attendu : modale ✅, passage en `ModeratorView`, mais `table_assignments` pointe désormais sur l'autre table et les actions d'animation échouent sur la table où il est physiquement (RLS, sans message). Vérifier aussi la variante « aucune table animée libre » : il reste sur place avec le flag → retombe sur le cas A.
- [x] **Écart C2 — asymétrie en phase `allocating`.** ✅ Corrigé au chantier 108, voir son entrée ci-dessous. *Note en cours de correction : le message du chantier 95 vivait dans `AllocatingScreen.tsx`, qui ne se monte en pratique que si la phase a déjà atteint `debating` (jamais pour un membre resté en `allocating` — celui-ci reste sur `VoteScreen` avec sa bannière). Le seul cas réel où `currentSession.phase === 'allocating'` s'y affiche est un retour en arrière de phase par le superadmin pendant qu'un participant est déjà sur cet écran (Realtime `sessions` UPDATE). Le chemin « Outils → Me déclarer modérateur » (`ModeratorClaimModal`, chantier 73), lui, a toujours été ouvert pendant `allocating` — la déclaration n'était donc pas réellement fermée aux déjà-inscrits en pratique, seul ce message resté figé après le chantier 95 disait le contraire.*
- [x] **Écart C1 — reconquête pré-vote qui perd la déclaration.** ✅ Corrigé et vérifié au navigateur au chantier 108, voir son entrée ci-dessous.
- [ ] **Écart C4 — questionnaire invisible pour le modérateur.** Passer une séance de `debating` à `post_voting` pendant qu'un modérateur est sur `ModeratorView` : attendu, l'overlay « La séance est terminée » recouvre le questionnaire forcé (les deux en `z-50`), contrairement à `ParticipantView` qui a la garde `!forcedQOpen`.
- [ ] **Écart C5 — cul-de-sac du retardataire « table seulement ».** Rejoindre une table par `JoinTableForm` sans jamais s'inscrire à la séance, puis passer la séance en `post_voting` : attendu, écran « le débat vient de se terminer », sans questionnaire ni résultats ni revote.
- [ ] **Piège D1 — perte de l'identité anonyme.** Difficile à reproduire fidèlement (il faut un renouvellement réel de l'`auth.uid()` anonyme). Approximation acceptable : effacer la session Supabase du `localStorage` en gardant la clé `tableStore`, recharger. Attendu : retour en `ParticipantView` pour un modérateur, sans aucun moyen de reprendre la main depuis cet écran.

---

## Chantier 108 — Harmoniser la déclaration modérateur sur tous les points d'entrée (2026-09-21)

Trois écarts (C1, C2, C3) de l'audit 87/101, tous fermés **après** le chantier 107 (`claim_moderator_status` ne déplace plus personne déjà assis — la déclaration ne pose plus qu'un drapeau pendant `allocating`).

**Vérifié au navigateur, sur une séance jetable créée et détruite en base pour ce test** (`C108TST`, 0 ligne restante après coup — vérifié) :

- [x] **C1 — reconquête pré-vote (`PseudoForm.tsx`).** Membre « Alice Test » inscrit en `pre_voting` (code de rappel noté). Depuis une identité anonyme neuve : nom identique + case « Je suis modérateur » cochée + mot de passe **volontairement invalide** → bascule bien en écran de reconquête (nom déjà pris) → code de rappel correct saisi → `tryClaimModeratorStatus` est bien rejoué après le reclaim réussi (avant ce chantier : silencieusement perdu) → écran « Tes votes ont bien été récupérés, mais la déclaration modérateur a échoué : Code Ecclesia invalide » → Continuer → atterrit directement sur l'écran de vote (bypass de l'écran code, comportement voulu, `onReclaimSuccess`). **Non rejoué avec le vrai Code Ecclesia** (pas en possession de cette session) : le succès RPC lui-même (`tryClaimModeratorStatus` retournant un membre) n'est pas nouveau dans ce chantier, seul le fait de le *rejouer après reclaim* l'est — la garde testée ici (appel bien déclenché, erreur bien affichée) couvre le risque réel du bug (silence).
- [x] **C2 — réouverture en `allocating` (`AllocatingScreen.tsx`).** Membre « Bob Test » inscrit en `voting` (attendance confirmée), séance passée en `debating` (arrivée sur `AllocatingScreen` via le formulaire de secours, cf. C3 ci-dessous), puis **repassée en `allocating`** pendant que Bob a l'écran ouvert : le nouveau formulaire (case + Code Ecclesia) apparaît bien par Realtime (`sessions` UPDATE → `setCurrentSession`), sans reload. Mot de passe volontairement invalide → `tryClaimModeratorStatus` appelée, message d'erreur rouge affiché, formulaire reste utilisable. **Point de conception noté dans ce fichier plus haut** (section audit 87/101) : ce message ne s'affiche en pratique que si le superadmin revient en arrière depuis `debating` — un membre qui n'a jamais dépassé `allocating` reste sur `VoteScreen`, où « Outils → Me déclarer modérateur » a toujours été ouvert (chantier 73, jamais fermé pendant `allocating`). Le fix est correct et suit exactement le périmètre demandé ; il couvre un cas réel mais plus rare que ce que l'audit supposait.
- [x] **C3 — case modérateur sur le formulaire de secours (`TableAssignmentCard.tsx`).** Bob Test, `debating`, sans affectation → formulaire de secours (code à 6 caractères) affiche désormais la case « Je suis modérateur de cette table ». Table jetable créée en base (`T108AB`) pour le test, code saisi + Code Ecclesia volontairement invalide → `claim_table_as_moderator` bien appelée via `handleSwitchAsModerator`, message serveur « Code Ecclesia incorrect » affiché correctement (chemin déjà existant quelques lignes plus haut dans le même composant, juste étendu à ce formulaire-ci).

**Reste à vérifier humainement, avec le vrai Code Ecclesia** (aucune des trois recettes ci-dessus n'a pu aller jusqu'au succès, faute de mot de passe) :
