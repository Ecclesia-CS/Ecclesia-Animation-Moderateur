...
// Chantier 27 (H22) — modale contrôlée (isOpen/onClose), intégrée comme entrée
// "Camps" du menu "Outils Modo" unifié (auparavant un bouton séparé du header).
export default function TableOpinionModal({ isOpen, onClose }: Props) {
  ...
  useEffect(() => {
    if (!isOpen || summary || loading) return
    setLoading(true)
    setError(null)
  ...
  if (!isOpen || !table.session_id) return null
