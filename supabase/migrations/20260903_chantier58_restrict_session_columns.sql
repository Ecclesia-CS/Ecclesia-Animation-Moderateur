...
--   - `onboarding_enabled` : colonne ajoutée par le chantier 71
--     (`20260904_chantier71_onboarding_optionnel.sql`), postérieur à
--     l'écriture initiale de cette migration sur la branche
--     `chantier-58-colonnes-sessions`. Ajoutée à la liste des colonnes
--     accordées avant application — sans ça, `VoteScreen.tsx` ne peut
--     plus lire `onboarding_enabled` sur une lecture directe de
--     `sessions` (l'interrupteur d'onboarding cesserait de fonctionner
--     côté participant). Note du chantier 71 sur le sujet, vérifiée
--     avant d'agir : les RPC `get_session_by_id`/`get_session_by_join_code`/
--     `list_sessions_admin` ci-dessous (SECURITY DEFINER, `RETURNS
--     sessions`/`SETOF sessions`) ne sont de toute façon pas concernées
--     par ce GRANT — colonne non sensible (simple bascule d'onboarding).
--
...
GRANT SELECT (id, title, phase, join_code, scheduled_at, created_at, onboarding_enabled)
  ON sessions TO anon, authenticated;

-- =============================================================
-- ROLLBACK (à exécuter en un bloc si besoin de revenir en arrière) :
--
-- REVOKE SELECT (id, title, phase, join_code, scheduled_at, created_at, onboarding_enabled)
...
--    GET /rest/v1/sessions?select=onboarding_enabled           -- attendu 200 (chantier 71)
--
